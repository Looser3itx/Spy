...
public = false
--if true will use /me to stand outw
publicItalics = false
...
local function onChatted(p,msg)
	if _G.chatSpyInstance == instance then
		if p==player and msg:lower():sub(1,6)=="/e spy" then
			enabled = not enabled
			wait(0.3)
			privateProperties.Text = "{Watcher "..(enabled and "EN" or "DIS").."ABLED}"
		...
		elseif enabled and (spyOnMyself==true or p~=player) then
			...
			if hidden and enabled then
				if public then
					saymsg:FireServer((publicItalics and "/me " or '').."{Watcher} [".. p.Name .."]: "..msg,"All")
				else
					privateProperties.Text = "{Watcher} [".. p.Name .."]: "..msg
					StarterGui:SetCore("ChatMakeSystemMessage",privateProperties)
				end
			end
		end
